    ...
    <div class="PoliticsInfoBox">
      <div class="InfoWrap">
        ...
        <div class="selectTime">
          ...
          <img src="../../assets/img/tianjia.png" alt style="margin-left:10px" @click="goAdd"/>
        </div>

        <div class="container">
          <div class="pageView common">
            <icon></icon>
            <a href="http://172.29.1.3:8091" style="color:white;font-weight:500">数据总量</a>
            <div class="view_box">
...
<script>
  ...
  import * as data from "../../APIs/data.js";
  ...
  export default {
    ...
    data() {
      return {
        ...
        //   "Wed Jul 31 2019 00:00:00 GMT+0800"
        // ], //时间段选择
        timevalue: ["2010-01-13", "2020-01-13"], //时间段选择
        warningList: [],
    ...
    methods: {
      async workNewsetData() {
        // that.dataBox = response.data.data.resultList;
        const receive = await data.getNewsetData();
        this.dataBox = receive.data.data.resultList;
        this.upMapDate();
        let _this = this;
        timer = setInterval(() => {
          _this.timevalue[0] = new Date(
      ...
      async workVisitCount() {
        const received2 = await data.getVisitCount()
        this.visitCount = received2.data;
        console.log(this.visitCount);
        let visitCountList = received2.data.data.resultList;
        for (var i = 0; i < visitCountList.length; i++) {
          if (visitCountList[i].type == 1) {
            this.locationVist.push(visitCountList[i]);
          } else if (visitCountList[i].type == 2) {
            this.lanVisit.push(visitCountList[i]);
          }
        }
        console.log(this.locationVist);
        console.log(this.lanVisit);
      },
      ...
      // 去涉政信息详情页或群体事件详情页
      //非常重要，连接两个组件
      // (item,index) in infoArr   把item传递过去
      goDetail(item) {
        if (item.type == 2) {
          this.$router.push({
            path: "/policyanalyze",
