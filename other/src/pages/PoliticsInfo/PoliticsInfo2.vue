...
export default {
  ...
  created() {
     ...
     axios.get('http://127.0.0.1:5000/newsetData')
    ...
    this.getZheDate();
    // dataBox = newsetDate.data.resultList;
  },
  data() {
    return {
      ...
      //   "Wed Jul 31 2019 00:00:00 GMT+0800"
      // ], //时间段选择
      timevalue: ["2019-01-13", "2020-01-13"], //时间段选择
      warningList: [],
  ...
  methods: {
   async getVisitCount(){
     const visitCount = await data.getVisitCount().data.data.resultList;
     // let visitCount = response.data.data.resultList;
      for (var i = 0; i < visitCount.length; i++) {
      if (visitCount[i].type == 1) {
        that.locationVist.push(visitCount[i]);
      } else if (visitCount[i].type == 2) {
        that.lanVisit.push(visitCount[i]);
      }
      }
   },
